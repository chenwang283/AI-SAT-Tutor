...
function getFigureImageUrl(figure) {
  const dataUrl =
    typeof figure?.dataUrl === "string" ? figure.dataUrl.trim() : "";
...
function cleanFigureForPrompt(figure) {
  if (!figure || typeof figure !== "object" || Array.isArray(figure))
    return null;
  const src =
    typeof figure.src === "string" && isHttpImageUrl(figure.src.trim())
      ? figure.src.trim()
      : null;
  const alt =
    typeof figure.alt === "string" && figure.alt.trim()
      ? figure.alt.trim()
      : null;
  const capturedImage = Boolean(getFigureImageUrl(figure));
  if (!src && !alt && !capturedImage) return null;
  return {
    src,
    alt,
...
function prepareQuestionForTutor(question) {
  ...
  return {
    question: {
      ...
      hasFigure: Boolean(cleanedFigures.length || images.length),
    },
    images,
  };
}
...
function getRequestConversation(body, { allowAssistantTail = false } = {}) {
  if (!Array.isArray(body?.conversation)) {
    throw requestError(
      400,
      "INVALID_REQUEST",
      "Request body must include a conversation array.",
    );
  }
  const conversation = body.conversation
    .map(cleanConversationMessage)
    .filter(Boolean);
  if (
    conversation.length &&
    conversation.at(-1).role !== "student" &&
    !allowAssistantTail
  ) {
    throw requestError(
      400,
      "INVALID_REQUEST",
      "A follow-up conversation must end with a student message.",
    );
  }
  return conversation;
}
...
function getRequestReviewChange(body) {
  ...
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw requestError(
      400,
      "INVALID_REQUEST",
      "reviewChange must be an object.",
    );
  }
  ...
  const changedFields = Array.isArray(value.changedFields)
    ? [
        ...new Set(
          value.changedFields.filter((field) => allowedFields.has(field)),
        ),
      ]
    : [];
  if (!changedFields.length) {
    throw requestError(
      400,
      "INVALID_REQUEST",
      "reviewChange must name at least one changed field.",
    );
  }
    ...
    for (const [source, target] of [
      ...
      if (!text || text.length > 4000) {
        throw requestError(
          400,
          "INVALID_REQUEST",
          `reviewChange ${field} is missing or too long.`,
        );
      }
      target[field] = text;
    }
...
app.get("/auth/confirmed", (req, res) =>
  res.sendFile(path.join(PUBLIC_DIR, "auth-confirmed.html")),
);
app.get("/auth/reset", (req, res) =>
  res.sendFile(path.join(PUBLIC_DIR, "auth-reset.html")),
);
...
app.post(
  "/auth/login",
  asyncRoute(async (req, res) =>
    res.json({ session: await authService.signIn(req.body || {}) }),
  ),
);
app.post(
  "/auth/refresh",
  asyncRoute(async (req, res) =>
    res.json({ session: await authService.refreshSession(req.body || {}) }),
  ),
);
app.post(
  "/auth/logout",
...
app.get(
  ...
  requireAuthenticatedUser,
  asyncRoute(async (req, res) =>
    res.json(await getDueReviews(req.auth.supabase, req.query.today)),
  ),
);
app.get(
  "/reviews/:id",
...
app.post("/teach", requireAuthenticatedUser, async (req, res) => {
  ...
  const reviewId =
    typeof req.body?.reviewId === "string" ? req.body.reviewId : null;
  ...
  try {
    if (!reviewId)
      throw requestError(400, "INVALID_REQUEST", "reviewId is required.");
    ...
    if (getQuestionKey(question) !== reviewRow.question_key) {
      throw requestError(
        409,
        "QUESTION_MISMATCH",
        "Open the question saved with this review before tutoring.",
      );
    }
