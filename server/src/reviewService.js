...
function cleanString(value, label, maxLength = 2000) {
  ...
  if (!cleaned) throw requestError(400, "INVALID_REVIEW", `${label} is required.`);
...
function normalizeSection(value) {
  ...
  throw requestError(400, "INVALID_REVIEW", "Section must be Math or Reading & Writing.");
}

function normalizeDifficulty(value) {
  const normalized = String(value || "").toLowerCase().trim();
  const aliases = { e: "easy", easy: "easy", m: "medium", medium: "medium", h: "hard", hard: "hard" };
  const difficulty = aliases[normalized];
  if (!difficulty) {
    throw requestError(400, "INVALID_REVIEW", "Difficulty must be Easy, Medium, or Hard.");
  }
  return difficulty;
}
...
function validateDateOnly(value, label = "Date") {
  ...
  if (!match) throw requestError(400, "INVALID_REVIEW", `${label} must use YYYY-MM-DD.`);
  ...
  if (
    ...
    throw requestError(400, "INVALID_REVIEW", `${label} is not a valid calendar date.`);
  }
  return date;
}
...
function validateTimezone(value) {
  ...
  try {
    ...
    throw requestError(400, "INVALID_REVIEW", "Timezone must be a valid IANA timezone.");
  }
  return timezone;
}
...
function getQuestionKey(question) {
  ...
  const questionType = typeof question?.questionType === "string" ? question.questionType.trim() : "unknown";
  const stem = typeof question?.stem === "string" ? question.stem.replace(/\s+/g, " ").trim() : "";
  if (!stem) throw requestError(400, "INVALID_QUESTION", "Captured question must include a stem.");
  const digest = createHash("sha256").update(`${questionType}\n${stem}`).digest("hex");
  return `content:${digest}`;
}

function deriveQuestionResult(question) {
  if (question?.questionType === "multiple_choice") {
    const selected = typeof question.selectedLetter === "string" ? question.selectedLetter.trim() : "";
    const correct = typeof question.correctLetter === "string" ? question.correctLetter.trim() : "";
    if (!selected || !correct) {
      throw requestError(
        400,
    ...
    return selected.toUpperCase() === correct.toUpperCase() ? "correct" : "wrong";
  }
  ...
  throw requestError(400, "INVALID_QUESTION", "This question type is not supported.");
}
...
function normalizeReviewInput(body, studentId) {
  ...
  if (!question || typeof question !== "object" || Array.isArray(question) || !question.stem) {
    throw requestError(400, "INVALID_QUESTION", "A captured StudySpaces question is required.");
  }
  ...
  const clockMode = String(diagnosis.clockMode || "").toLowerCase().trim();
  if (!["timed", "untimed"].includes(clockMode)) {
    throw requestError(400, "INVALID_REVIEW", "Clock must be Timed or Untimed.");
  }
  ...
  return {
    ...
    question_number: cleanString(metadata.questionNumber, "Question number", 50),
    section,
...
function serializeReview(row) {
  ...
  return {
    ...
    tag: row.mistake_tag,
    tagDefinition: getTagDefinition(row.section, row.mistake_tag)?.description || null,
    redo3: {
...
function databaseError(error, fallbackMessage) {
  if (error?.code === "23505") {
    return requestError(409, "DUPLICATE_REVIEW", "This question already has a review logged for this date.");
  }
...
async function createReview(supabase, studentId, body) {
  ...
  const { data, error } = await supabase.from("question_reviews").insert(row).select("*").single();
...
async function getOwnedReview(supabase, id) {
  const { data, error } = await supabase.from("question_reviews").select("*").eq("id", id).maybeSingle();
...
function buildDueSummary(rows, today) {
  const items = rows
    ...
    .filter(Boolean)
    .sort((a, b) => a.dueOn.localeCompare(b.dueOn) || a.source.localeCompare(b.source));
...
async function completeRedo(supabase, id, stageValue, body) {
  ...
  if (![3, 14].includes(stage)) {
    throw requestError(400, "INVALID_REDO_STAGE", "Redo stage must be 3 or 14.");
  }
  ...
  if (getQuestionKey(question) !== row.question_key) {
    throw requestError(409, "QUESTION_MISMATCH", "Open the saved StudySpaces question before completing this redo.");
  }
  ...
  const completedField = stage === 3 ? "redo_3_completed_at" : "redo_14_completed_at";
  const dueField = stage === 3 ? "redo_3_due_on" : "redo_14_due_on";
  if (row[resultField]) throw requestError(409, "REDO_COMPLETE", "This redo is already complete.");
  if (today < row[dueField]) throw requestError(409, "REDO_NOT_DUE", "This redo is not due yet.");

  const result = deriveQuestionResult(question);
  const { data, error } = await supabase
    .from("question_reviews")
    .update({ [resultField]: result, [completedField]: new Date().toISOString() })
    .eq("id", id)
    .is(resultField, null)
    .select("*")
    .maybeSingle();
  if (error) throw databaseError(error, "Unable to complete the redo.");
  if (!data) throw requestError(409, "REDO_COMPLETE", "This redo is already complete.");
